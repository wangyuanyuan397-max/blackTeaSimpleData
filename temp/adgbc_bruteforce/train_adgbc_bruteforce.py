"""Queue AD-GBC brute-force configs through the shared batch trainer."""
...
COMMON_CONFIG = Path("temp/adgbc_bruteforce/common_01234_basic_408_train.yaml")
CONFIG_ROOT = Path("temp/adgbc_bruteforce/configs")
RUNS_ROOT = PROJECT_ROOT / "temp/adgbc_bruteforce/runs_BaSic"
...
# AD-GBC 暴力搜索实验数量很大，统一禁止保留 .pth 权重文件。
...
def parse_local_args(argv: list[str]) -> tuple[argparse.Namespace, list[str]]:
    ...
    parser.add_argument(
        ...
        default=("smoke",),
        help="AD-GBC config phase to run before forwarding remaining args.",
    )
    parser.add_argument(
        "--config-dir",
        type=Path,
    ...
    parser.add_argument(
        "--generate",
        action="store_true",
        help="Regenerate AD-GBC YAML configs before collecting them.",
    )
    parser.add_argument(
        "--no-summary",
        action="store_true",
...
def enforce_discard_pth(args: list[str]) -> list[str]:
    # 如果用户误传保留权重的参数，直接报错；否则自动追加 --discard-pth。
    if any(arg in KEEP_PTH_FLAGS for arg in args):
        raise ValueError("AD-GBC brute-force runs discard .pth files; remove --keep-pth.")
...
def main() -> None:
    ...
    if local_args.generate:
        from generate_adgbc_configs import main as generate_main

        old_argv = sys.argv
        try:
            generate_phase = "all" if "all" in local_args.phase else local_args.phase[0]
            sys.argv = ["generate_adgbc_configs.py", "--phase", generate_phase]
            generate_main()
        finally:
            sys.argv = old_argv
    ...
    # PyCharm/脚本入口也强制不保留权重，和命令行 --discard-pth 保持一致。
    ...
    try:
        ...
        if summarize_after_run and RUNS_ROOT.exists():
            from summarize_adgbc_results import summarize_results
